...
def create_hcs_topic(client, topic_name, memo):
    ...
    try:
        ...
        # Create topic transaction (without setTransactionValidDuration to avoid clock sync issues)
        transaction = (
            TopicCreateTransaction()
            .setTopicMemo(memo)
...
def check_operator_balance(client, operator_id):
    ...
    Args:
        client: Hedera Client instance
        operator_id: Operator account ID (string or AccountId)
        
    Returns:
        float: Balance in HBAR
    """
    from hedera import AccountId, AccountBalanceQuery
    ...
    try:
        ...
        print(f"[OK] Operator balance: {hbar_balance} HBAR")
